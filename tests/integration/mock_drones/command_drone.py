...
def main() -> int:
    ...
    for _ in range(NUM_TRIALS):
        ...
        if not msg or msg.get_type() != "COMMAND_LONG":
            local_logger.error(
                "Sent incorrect message type or timed out, still expecting mesages"
                + f"Received message: {msg.get_type() if msg else None}"
            )
            return -2
        if msg.command not in (
            ...
            local_logger.error(
                "Sent incorrect command within COMMAND_LONG message. "
                + f"Received command: {msg.command}"
            )
            return -3
